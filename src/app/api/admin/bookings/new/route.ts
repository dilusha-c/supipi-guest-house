...
export async function POST(req: Request) {
  try {
    ...
    // Double Booking Prevention: Check against existing CONFIRMED bookings
    ...
    if (conflict) {
      return NextResponse.json({
        error: `Cannot create booking: dates conflict with confirmed booking (${conflict.bookingReference}: ${conflict.guestName}).`
      }, { status: 409 });
    }

    const dateStr = new Date().toISOString().split('T')[0].replace(/-/g, '');
    const randomSuffix = crypto.randomBytes(2).toString('hex').toUpperCase();
    const bookingReference = `SP-${dateStr}-${randomSuffix}`;

    let parsedPrice: number | null = null;
    if (price !== undefined && price !== null && price !== '') {
      const p = typeof price === 'number' ? price : parseFloat(String(price).replace(/[^0-9.]/g, ''));
      if (!isNaN(p) && Number.isFinite(p) && p >= 0) {
        parsedPrice = p;
      }
    }
    ...
    const newBooking = await prisma.booking.create({
      data: {
        ...
        children: Math.max(0, parseInt(children || '0') || 0),
        status: 'CONFIRMED', 
    ...
    if (newBooking.email && newBooking.email.trim() !== '') {
      const safeGuest = escapeHtml(newBooking.guestName);
      const safeRef = escapeHtml(bookingReference);
      const safePrice = parsedPrice !== null ? `LKR ${parsedPrice.toLocaleString()}` : '';
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 25px;">
              <tbody>
                <tr>
                  ...
                  <td style="padding: 10px 0; border-bottom: 1px solid #eeeeee; text-align: right; color: #555;">${checkInDate.toLocaleDateString()} (After 2:00 PM)</td>
                </tr>
                <tr>
                  <td style="padding: 10px 0; border-bottom: 1px solid #eeeeee; color: #888888;">Check-out:</td>
                  <td style="padding: 10px 0; border-bottom: 1px solid #eeeeee; text-align: right; color: #555;">${checkOutDate.toLocaleDateString()} (Before 11:00 AM)</td>
                </tr>
                <tr>
                  <td style="padding: 10px 0; border-bottom: 1px solid #eeeeee; color: #888888;">Guests:</td>
                  <td style="padding: 10px 0; border-bottom: 1px solid #eeeeee; text-align: right; color: #555;">${newBooking.adults} Adults, ${newBooking.children} Children</td>
                </tr>
                ${parsedPrice !== null ? `
                <tr>
      ...
      try {
        ...
        console.error("Email sending failed for manual booking:", mailErr);
      }
    }
